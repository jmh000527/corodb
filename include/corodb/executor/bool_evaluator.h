...
namespace corodb {
    ...
    public:
        ...
        /** @brief 求值布尔表达式（AND/OR/NOT/IN/BETWEEN）；subq 非空时支持相关子查询逐行求值。 */
        [[nodiscard]] static SqlBool eval(const BoolExpr& expr, const Record& record, SubqueryRunner* subq = nullptr);
    };
